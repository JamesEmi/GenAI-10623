...
class TextClassificationModel(nn.Module):
    def __init__(self, vocab_size, embed_dim, num_class, hidden_dim=128, num_layers=2, dropout=0.2):
...
def train_one_epoch(dataloader, model, criterion, optimizer, epoch):
    ...
    for idx, (text, label) in enumerate(dataloader):
        ...
        # print(f"Predicted shape: {predicted_label.shape}, Label shape: {label.shape}")
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        train_loss = train_one_epoch(train_dataloader, model, criterion, optimizer, epoch)
