...
def get_data():    
    ...
    # Temporary DataFrame for creating CorpusInfo
    temp_df = pd.read_csv('./data/txt_train.csv')
    temp_data = [(text, 0) for text in temp_df["article"]]  # Create temporary data list

    corpus_info = CorpusInfo(temp_data, tokenizer)
...
def main():
    ...
    # Load data just for calculating lengths
    temp_train_data = pd.read_csv('./data/txt_train.csv')
    article_lengths = [len(tokenizer(text)) for text in temp_train_data["article"]]

    wandb.init(project="text_classification",config={
        "epochs": EPOCHS,
        "learning_rate": LR,
    ...
    # Log the histogram of article lengths
    wandb.log({"article_lengths": wandb.Histogram(article_lengths)})
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        wandb.log({"epoch": epoch, "train_loss": train_loss, "val_accuracy": accu_val})
