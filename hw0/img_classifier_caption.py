...
label_mapping = {0: "parrot", 1: "narwhal", 2: "axolotl"}

def train_one_epoch(dataloader, model, loss_fn, optimizer, t, current_epoch, total_epochs):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        wandb.log({"batch_loss": loss, "examples_processed": current}) #check if this position is ok
        if batch % 10 == 0:
            print(f"Train loss = {loss:>7f}  [{current:>5d}/{size:>5d}]")

        if current_epoch == total_epochs - 1 and batch == 0:
            predictions = pred.argmax(1)
            captions = [f"{label_mapping[pred.item()]} / {label_mapping[true.item()]}" for pred, true in zip(predictions, y)]
            processed_images = [wandb.Image(image.permute(1,2,0).cpu().numpy(), caption=caption) for image, caption in zip(X, captions)]
            wandb.log({"examples": processed_images})
        
        
def evaluate(dataloader, dataname, model, loss_fn, t, total_epochs):
    size = len(dataloader.dataset)
    ...
    with torch.no_grad():
        for batch, (X, y) in enumerate(dataloader):
            ...
            # Log images for the first batch of the last epoch
            if t == total_epochs - 1 and batch == 0:
                predictions = pred.argmax(1)
                captions = [f"{label_mapping[pred.item()]} / {label_mapping[true.item()]}" for pred, true in zip(predictions, y)]
                processed_images = [wandb.Image(image.permute(1,2,0).cpu().numpy(), caption=caption) for image, caption in zip(X, captions)]
                wandb.log({f"{dataname}_examples": processed_images})
    ...
    wandb.log({
        f"{dataname}_accuracy": 100 * correct,
        f"{dataname}_avg_loss": avg_loss,
        "epoch": t
    })
...
def main(n_epochs, batch_size, learning_rate):
    ...
    for t in range(n_epochs):
        print(f"\nEpoch {t+1}\n-------------------------------")
        train_one_epoch(train_dataloader, model, loss_fn, optimizer, t, t, args.n_epochs)
        evaluate(train_dataloader, "Train", model, loss_fn, t, args.n_epochs)
        evaluate(test_dataloader, "Test", model, loss_fn, t, args.n_epochs)
        evaluate(val_dataloader, "Val", model, loss_fn, t, args.n_epochs)
